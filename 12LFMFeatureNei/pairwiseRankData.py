...
def readData(inFile,outFile,neighborFeature):
    fin = open(inFile,"r")    # 
    fout = open(outFile,'w')
    count = 0
    for current in fin:
        ...
        fout.write(str(numRT1)+'\t'+str(globalNum) +'\t'+ '1\t1' +'\t'+ neighborFeature[tweetId] + str(count) + ':1' +'\t'+ str(hourTime1) + ':1'  +'\t'+firstData+'\n')
...
def main(argv):
    ...
    readData(inFile,outFile,neighborFeature)
